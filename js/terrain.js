...
export class Terrain {
  constructor(size = 192) {
    ...
    this.paint = new Float32Array(size * size);
    this.snow = new Float32Array(size * size);
    this.veg = new Float32Array(size * size);
    this.noise = new PerlinNoise(1337);

    this.params = {
      freq: 0.50, amp: 1.60, oct: 6, seed: 1337,
      ridge: 0.60, peak: 0.50, erode: 0.20,
      plateau: 0.00, smooth: 0.05,
    };
  }
  ...
  generate() {
    const { freq, amp, oct, ridge, peak } = this.params;
    const s = this.size;
    ...
    for (let y = 0; y < s; y++) {
      for (let x = 0; x < s; x++) {
        ...
        const ny = y / s;
        const f = freq * 4;
        const fbm = this.noise.fbm(nx * f, ny * f, oct) * 0.5;
        const rid = this.noise.ridged(nx * f * 0.8, ny * f * 0.8, Math.max(2, oct - 1));
        // Blend fbm and ridged based on ridge param
        let v = fbm * (1 - ridge) + rid * ridge * 0.8;
        v = (v + 0.5) * amp;
        h[y * s + x] = Math.max(0, v);
      }
    }

    this._normalize();

    // Peak exaggeration — sharp spires like Bryce reference
    if (peak > 0.01) {
      const power = 1 + peak * 2.5;
      for (let i = 0; i < h.length; i++) {
        h[i] = Math.pow(h[i], power);
      }
      this._normalize();
    }
  ...
  _applySmooth(amount) {
    ...
    for (let p = 0; p < passes; p++) {
      for (let y = 1; y < s - 1; y++) {
        for (let x = 1; x < s - 1; x++) {
          const i = y * s + x;
          tmp[i] = (h[i] + h[i - 1] + h[i + 1] + h[i - s] + h[i + s]) / 5;
        }
      }
      for (let y = 1; y < s - 1; y++) {
        for (let x = 1; x < s - 1; x++) {
  ...
  samplePaint(layer, u, v) {
    ...
    const x0 = Math.floor(px), y0 = Math.floor(py);
    const fx = px - x0, fy = py - y0;
    const i00 = y0 * s + x0, i10 = i00 + 1, i01 = i00 + s, i11 = i01 + 1;
