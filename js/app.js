...
document.querySelectorAll('.world-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    $('mallControls').style.display = (mode === 'mall') ? '' : 'none';
    log(`world · ${mode}`);
  });
});
...
// MALL controls
function pushMallControlsFromState() {
  if (!scene.world.mall) return;
  const p = scene.world.mall.params;
  $('mallColA').value = intToHex(p.floorColorA);
  $('mallColB').value = intToHex(p.floorColorB);
  $('mallColC').value = intToHex(p.floorColorC);
  $('mallNeon').value = intToHex(p.neonColor);
  $('mallAmbi').value = intToHex(p.ambientTint);
  $('mallStore').value = p.storeDensity * 100; $('mallStoreV').textContent = p.storeDensity.toFixed(2);
  $('mallMann').value = p.mannequinDensity * 100; $('mallMannV').textContent = p.mannequinDensity.toFixed(2);
  $('mallPalms').value = p.palmCount; $('mallPalmsV').textContent = p.palmCount;
  $('mallEsc').checked = p.escalatorEnabled;
  $('mallCols').checked = p.columnsEnabled;
  $('mallNeonStrips').checked = p.neonStrips;
  $('mallFlicker').checked = p.flickerEnabled;
  document.querySelectorAll('[data-mallpattern]').forEach(b => {
    b.classList.toggle('active', b.dataset.mallpattern === p.floorPattern);
  });
  document.querySelectorAll('[data-mallpreset]').forEach(b => {
    b.classList.toggle('active', b.dataset.mallpreset === p.preset);
  });
}

document.querySelectorAll('[data-mallpreset]').forEach(btn => {
  btn.addEventListener('click', () => {
    if (scene.world.mode !== 'mall') return;
    scene.world.setMallPreset(btn.dataset.mallpreset);
    document.querySelectorAll('[data-mallpreset]').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    pushMallControlsFromState();
    log(`mall · ${btn.dataset.mallpreset}`);
  });
});

document.querySelectorAll('[data-mallpattern]').forEach(btn => {
  btn.addEventListener('click', () => {
    if (scene.world.mode !== 'mall') return;
    scene.world.setMallParam('floorPattern', btn.dataset.mallpattern);
    document.querySelectorAll('[data-mallpattern]').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
  });
});

$('mallColA').addEventListener('input', e => scene.world.setMallParam('floorColorA', hexToInt(e.target.value)));
$('mallColB').addEventListener('input', e => scene.world.setMallParam('floorColorB', hexToInt(e.target.value)));
$('mallColC').addEventListener('input', e => scene.world.setMallParam('floorColorC', hexToInt(e.target.value)));
$('mallNeon').addEventListener('input', e => scene.world.setMallParam('neonColor', hexToInt(e.target.value)));
$('mallAmbi').addEventListener('input', e => scene.world.setMallParam('ambientTint', hexToInt(e.target.value)));
bindSlider('mallStore', 'mallStoreV', 0.01, v => v.toFixed(2), v => scene.world.setMallParam('storeDensity', v));
bindSlider('mallMann', 'mallMannV', 0.01, v => v.toFixed(2), v => scene.world.setMallParam('mannequinDensity', v));
bindSlider('mallPalms', 'mallPalmsV', 1, v => v.toFixed(0), v => scene.world.setMallParam('palmCount', Math.round(v)));
$('mallEsc').addEventListener('change', e => scene.world.setMallParam('escalatorEnabled', e.target.checked));
$('mallCols').addEventListener('change', e => scene.world.setMallParam('columnsEnabled', e.target.checked));
$('mallNeonStrips').addEventListener('change', e => scene.world.setMallParam('neonStrips', e.target.checked));
$('mallFlicker').addEventListener('change', e => scene.world.setMallParam('flickerEnabled', e.target.checked));
...
function pushWorldControlsFromState() {
  ...
  $('mallControls').style.display = (scene.world.mode === 'mall') ? '' : 'none';
...
function pushAllControlsFromState() {
  ...
  if (scene.world.mode === 'mall') pushMallControlsFromState();
}
